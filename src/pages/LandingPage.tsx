  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative min-h-[100vh] flex items-center justify-center overflow-hidden">
        ...
        {/* Kanji decoration */}
        <div className="absolute top-8 right-8 text-white/5 text-[120px] font-serif leading-none select-none hidden md:block">
          武道
        </div>

        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          <img src={shizenLogo} alt="Dojo Shizen" className="w-24 h-24 mx-auto rounded-2xl shadow-2xl mb-8" />

          <p className="text-primary-foreground/60 text-sm tracking-[0.4em] uppercase font-medium mb-4">
          ...
          <h1 className="font-serif text-4xl md:text-6xl font-bold text-white leading-tight mb-6">
            Sua Academia no
            <span className="block text-primary" style={{ color: "hsl(0, 100%, 45%)" }}> Próximo Nível</span>
          </h1>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => {
              const Icon = f.icon;
              return (
                <div key={i} className="dojo-card p-6 hover:shadow-lg transition-shadow group">
                  <div
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="dojo-card p-6">
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, j) => (
                    <Star key={j} size={16} fill="hsl(var(--dojo-yellow))" style={{ color: "hsl(var(--dojo-yellow))" }} />
                  ))}
                </div>
